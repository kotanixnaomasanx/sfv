...
def _facet_bar(pub, exclude=()):
    """出展企業の市町・エリア・業種・参加形態のタグ絞り込みバーを生成。exclude で軸を除外できる。"""
    ...
    _groups = ""
    if cities and "city" not in exclude:      _groups += f'<div class="flt-g"><span class="flt-l">市町</span>{_chips("city", cities)}</div>'
    if areas and "area" not in exclude:       _groups += f'<div class="flt-g"><span class="flt-l">エリア</span>{_chips("area", areas)}</div>'
    if inds and "industry" not in exclude:    _groups += f'<div class="flt-g"><span class="flt-l">業種</span>{_chips("industry", inds)}</div>'
    if joinset and "joins" not in exclude:    _groups += f'<div class="flt-g"><span class="flt-l">参加形態</span>{_chips("joins", joinset)}</div>'
